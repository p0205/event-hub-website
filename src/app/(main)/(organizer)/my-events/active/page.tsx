...
export default function ActiveEventsPage() {
    ...
    useEffect(() => {
      ...
      const fetchActiveEvents = async () => {
        ...
        try {
          const activeEvents = await eventService.getEventsByStatus(Number(user?.id),EventStatus.ACTIVE);
          if (!activeEvents) {
            ...
            <ul className="event-list"> {/* Add a class for styling the list */}
                ...
                {activeEvents.map((event) => (
                    ...
                    <li key={event.id} className="event-list-item"> {/* Add a class for styling each list item */}
                        ...
                        <Link href={`/my-events/active/${event.id}`} className="event-list-link"> {/* Add a class for the link area */}
